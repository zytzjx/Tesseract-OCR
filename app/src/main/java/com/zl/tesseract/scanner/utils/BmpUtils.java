...
public class BmpUtils {
     ...
     * @return
     */
    static final String TAG = "DBG_BmpUtils";
    public static List<String> parseMultiCode(Bitmap bmp){
        BinaryBitmap bitmap = null;
        Log.d(TAG, "parseMultiCode: ImageSize="+bmp.getWidth()+"X"+bmp.getHeight());
        int[] pixels = new int[bmp.getWidth()*bmp.getHeight()];// = image.getRGB(0, 0, image.getWidth(), image.getHeight(), null, 0, image.getWidth());
        ...
        for (Result res: zxingResults) {
            Log.d(TAG, "parseMultiCode: " + res.getText());
            items.add(res.getText());

            Log.d(TAG,"Found " + res.getResultPoints().length + " result points.");
            for (int i = 0; i < res.getResultPoints().length; i++) {
                ResultPoint rp = res.getResultPoints()[i];
                Log.d(TAG,"  Point " + i + ": (" + rp.getX() + ',' + rp.getY() + ')');
            }
        }
        return items;
    }
